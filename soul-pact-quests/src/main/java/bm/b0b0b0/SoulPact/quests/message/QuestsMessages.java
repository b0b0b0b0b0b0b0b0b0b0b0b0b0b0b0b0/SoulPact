...
public final class QuestsMessages {
    ...
    public QuestsMessages(JavaPlugin plugin, String locale, String fallbackLocale) {
        this.plugin = plugin;
        this.locale = locale;
    ...
    public void load() {
        ...
        try (Stream<java.nio.file.Path> paths = Files.list(langFolder.toPath())) {
            ...
            plugin.getLogger().severe("Failed to load quests lang: " + exception.getMessage());
        }
    }
    ...
    public Component component(Player player, String key, Map<String, String> placeholders) {
        return QuestsTextParser.parse(resolve(player, key, placeholders));
    }
    ...
    private void copyDefault(String fileName, File langFolder) {
        ...
        try (InputStream stream = plugin.getResource("lang/" + fileName)) {
            ...
            plugin.getLogger().severe("Failed to copy quests lang " + fileName + ": " + exception.getMessage());
        }
    }
}
